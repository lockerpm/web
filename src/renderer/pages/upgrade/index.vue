  ...
  <div class="mt-10">
        ...
        >
          <div class="h-full flex flex-col">
            ...
            <div v-else-if="currentPlan.alias === 'pm_free'">
              <button class="btn btn-primary w-full">Hiện tại</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <template v-if="step === 2">
      ...
      <div class="grid grid-cols-4 gap-x-20 mb-12">
        <div class="col-span-3">
          <div class="mb-10">
            ...
            <div class="grid grid-cols-3 gap-x-6">
              <div
                ...
                :class="{
                  'border-primary': selectedPeriod.label === item.label
                }"
                @click="selectPeriod(item)"
              >
                <div class="font-semibold text-[1rem] mb-2.5">
                  {{ $t(`data.plans.price.${item.label}`) }}
                </div>
                <template v-if="selectedPlan[item.label]">
                  <div class="font-semibold mb-8">
                    <span>
                      <span class="text-head-1">{{ symbol
                      }}{{
                        (selectedPlan[item.label][currency] / item.value)
                          | formatNumber
                      }}</span>
                      <span> / Mo </span>
                      <span
                        v-if="selectedPlan.max_number"
                        class="text-black-600"
                      >/ {{ selectedPlan.max_number }} members
                      </span>
                      <span
                        v-else-if="selectedPlan.alias === 'pm_business_premium'"
                        class="text-black-600"
                      >/ 1 member
                      </span>
                    </span>
                  ...
                  <div v-if="item.value !== 1">
                    <span class="text-danger line-through">
                      {{ symbol
                        ...
                        (selectedPlan.price[currency] * item.value)
                          | formatNumber
                      }}</span>
                    <span class="text-black-600">
                      {{ symbol
                      }}{{ selectedPlan[item.label][currency] | formatNumber }}
                      <span v-if="item.value === 6">
                        for the first 6 months</span>
          ...
          <div
            v-if="selectedPlan.alias === 'pm_business_premium'"
            class="mb-10"
          >
            <div class="text-[20px] font-semibold mb-6">
          ...
          <div>
            ...
            <div class="grid grid-cols-3 gap-6">
              ...
              >
                ...
                <div class="text-center">
                  <a href="https://id.locker.io/wallet/vnd" target="_blank">
                    <button
                      class="btn rounded-full btn-xs hover:no-underline mt-auto !font-normal"
                    >
          ...
          <div
            ...
            style="
              background: linear-gradient(
                90deg,
                rgba(219, 223, 225, 0.16) 0%,
                rgba(219, 223, 225, 0) 100%
              );
            "
          >
            <div class="text-[20px] font-semibold mb-6">
            ...
            <div class="flex items-center justify-between mb-4">
              <div>
                ...
                <div>
                  ...
                  <span v-if="selectedPlan.id === 4">
                    x
                    {{
                      $tc('data.plans.members', number_members, {
                        count: number_members
                      })
                    }}</span>
                </div>
              </div>
              <div>{{ result.price | formatNumber }} {{ result.currency }}</div>
            </div>
            <div
              v-if="result.immediate_payment"
            ...
            <div
              class="flex items-center justify-between text-[20px] font-semibold"
            >
    ...
    >
      ...
      <div class="text-left text-black">
        ...
        <div class="mb-5">
          <div class="font-semibold mb-6">Choose subscription period</div>
          <div class="grid grid-cols-3 gap-x-6">
            <div
              v-for="item in periods"
              :key="item.value"
            ...
            >
              ...
              <template v-if="selectedPlan[item.label]">
                <div class="font-semibold">
                  <span>
                    <span class="text-head-5">{{ symbol
                    }}{{
                      (selectedPlan[item.label][currency] / item.value)
                        | formatNumber
                    }}</span>
                    <span> / Mo </span>
                    <div v-if="selectedPlan.max_number" class="text-black-600">
                      / {{ selectedPlan.max_number }} members
                    </div>
                    <div
                      v-else-if="selectedPlan.alias === 'pm_business_premium'"
        ...
        <div v-if="selectedPlan.alias === 'pm_business_premium'" class="mb-5">
          <div class="font-semibold mb-6">Choose number of members</div>
          <el-input-number
            v-model="number_members"
            controls-position="right"
        ...
        <div class="mb-5">
          <div class="font-semibold mb-5">Select a payment method</div>
          <div class="grid gap-5">
            <div
              v-if="
                currentPlan.payment_method === 'wallet' ||
                  currentPlan.payment_method === 'banking'
              "
        ...
        <div class="text-lg">
          <div class="font-semibold mb-6">Summary</div>
          <div
            v-if="result.next_billing_time"
            class="flex items-center justify-between"
          >
            <div>
              {{ $t('data.billing.next_billing') }}:
              <span class="font-semibold">{{
                $moment(result.next_billing_time * 1000).format('LL')
              }}</span>
            </div>
            <div>{{ result.price | formatNumber }} {{ result.currency }}</div>
          </div>
    ...
    >
      ...
      <div slot="footer" class="dialog-footer flex items-center text-left">
        <div class="flex-grow" />
        <div>
          <button class="btn btn-default" @click="dialogTopup = false">
            {{ $t('common.cancel') }}
          </button>
          <a
            href="https://id.locker.io/wallet/vnd"
  ...
  methods: {
    ...
    selectPlan (plan) {
      ...
      if (this.currentPlan.alias !== 'pm_free') {
        ...
        this.selectPeriod(
          find(this.periods, e => e.duration === this.currentPlan.duration)
        )
