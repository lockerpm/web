...
export class MyServices implements CipherServiceAbstraction {
         ...
         async encrypt(
           ...
           if (model.id != null) {
             ...
             if (originalCipher != null) {
               ...
               if (existingCipher.hasFields) {
                 ...
                 existingHiddenFields.forEach(ef => {
                   const matchedField = hiddenFields.find(
                     f => f.name === ef.name
                   );
                   if (
                     matchedField == null ||
                     matchedField.value !== ef.value
                   ) {
                     const ph = new PasswordHistoryView();
             ...
             if (
               ...
               model.passwordHistory.length === 0
             ) {
               model.passwordHistory = null;
             } else if (
               model.passwordHistory != null &&
           ...
           if (key == null && cipher.organizationId != null) {
             ...
             if (key == null) {
               throw new Error(
                 "Cannot encrypt cipher for organization. No key."
               );
             }
           }
           await Promise.all([
             ...
             this.encryptPasswordHistories(model.passwordHistory, key).then(
               ph => {
                 cipher.passwordHistory = ph;
               }
             ),
             this.encryptAttachments(model.attachments, key).then(
               attachments => {
         ...
         async encryptAttachments(
           ...
           attachmentsModel.forEach(async model => {
             ...
             const promise = this.encryptObjProperty(
               ...
               if (model.key != null) {
                 attachment.key = await this.cryptoService.encrypt(
                   model.key.key,
                   key
                 );
               }
         ...
         async encryptField(
           ...
           if (
             fieldModel.type === FieldType.Boolean &&
             fieldModel.value !== "true"
           ) {
         ...
         async getAllDecryptedForUrl(
           ...
           return ciphers.filter(cipher => {
             ...
             if (
               ...
               includeOtherTypes.indexOf(cipher.type) > -1
             ) {
               return true;
             }
                   ...
                   case UriMatchType.Domain:
                     if (
                       ...
                       if (DomainMatchBlacklist.has(u.domain)) {
                         ...
                         if (
                           !DomainMatchBlacklist.get(u.domain).has(
                             domainUrlHost
                           )
                         ) {
                           return true;
         ...
         async getAllFromApiForOrganization(
           ...
           const ciphers = await this.apiService.getCiphersOrganization(
             organizationId
           );
         ...
         async updateLastUsedDate(id: string): Promise<void> {
           let ciphersLocalData = await this.storageService.get<any>(
             Keys.localData
           );
           if (!ciphersLocalData) {
             ciphersLocalData = {};
         ...
         async shareWithServer(
           ...
           const response = await this.apiService.putShareCipher(
             cipher.id,
             request
           );
           const userId = await this.userService.getUserId();
         ...
         async shareManyWithServer(
           ...
           await Promise.all(promises);
           const request = new CipherBulkShareRequest(
             encCiphers,
             collectionIds
           );
         ...
         async deleteManyWithServer(ids: string[]): Promise<any> {
           await this.apiService.deleteManyCiphers(
             new CipherBulkDeleteRequest(ids)
           );
         ...
         async softDeleteManyWithServer(ids: string[]): Promise<any> {
           await this.apiService.putDeleteManyCiphers(
             new CipherBulkDeleteRequest(ids)
           );
           ...
           if (attachmentResponse.status !== 200) {
             throw Error(
               "Failed to download attachment: " +
                 attachmentResponse.status.toString()
             );
           }
