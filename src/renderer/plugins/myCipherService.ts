...
export class MyCipherService implements CipherServiceAbstraction {
  ...
  get decryptedCipherCache(): any[] {
    return this._decryptedCipherCache;
  }
  set decryptedCipherCache(value: any[]) {
  ...
  private async encryptCipherData(
    ...
    switch (cipher.type) {
      ...
      case CipherType.CryptoAccount:
        cipher.cryptoAccount = new CryptoAccount();
        await this.encryptObjProperty(
          model.cryptoAccount,
        ...
        if (model.login.uris != null) {
          cipher.cryptoAccount.uris = [];
          for (let i = 0; i < model.cryptoAccount.uris.length; i++) {
            const loginUri = new LoginUri();
            loginUri.match = model.login.uris[i].match;
            await this.encryptObjProperty(
              model.login.uris[i],
