export default ({ store, app }) => {
  ...
  app.router.beforeEach(async (toRoute, fromRoute, next) => {
    if (fromRoute && toRoute && fromRoute.path && toRoute.path && !fromRoute.path.includes('/login') && !toRoute.path.includes('/login') && !fromRoute.path.includes('/lock') && !toRoute.path.includes('/lock')
    ) {
