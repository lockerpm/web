  ...
  methods: {
    ...
    async getPublicKey (emergency_access) {
      ...
      const { public_key: publicKey } = await this.$axios.$get(
        `cystack_platform/pm/emergency_access/${emergency_access.id}/public_key`
      )
      return publicKey
    },
    async promptConfirmEmergencyAccess (emergency_access) {
      ...
      const fingerprint = await this.$cryptoService.getFingerprint(
        emergency_access.grantee_pwd_user_id,
        publicKey.buffer
      )
      if (fingerprint) {
    ...
    async confirmEmergencyAccess (emergency_access) {
      try {
        ...
        await this.$axios.$post(
          `cystack_platform/pm/emergency_access/${emergency_access.id}/confirm`,
          {
            key
          }
        )
    ...
    async requestAccess (emergency_access) {
      this.$confirm(
        this.$t('data.notifications.request_emergency_access', {
          day: emergency_access.wait_time_days
        }),
        ...
        .then(async () => {
          try {
            await this.$axios.$post(
              `cystack_platform/pm/emergency_access/${emergency_access.id}/initiate`
            )
    ...
    async approveEmergencyAccess (emergency_access) {
      this.$confirm(
        this.$t('data.notifications.approve_emergency_access', {
          user: emergency_access.full_name,
        ...
        .then(async () => {
          try {
            await this.$axios.$post(
              `cystack_platform/pm/emergency_access/${emergency_access.id}/approve`
            )
            this.getListTrusted()
            this.notify(
    ...
    async rejectEmergencyAccess (emergency_access) {
      try {
        await this.$axios.$post(
          `cystack_platform/pm/emergency_access/${emergency_access.id}/reject`
        )
        this.getListTrusted()
        this.notify(
    ...
    async acceptInvite (emergency_access) {
      try {
        await this.$axios.$post(
          `cystack_platform/pm/emergency_access/${emergency_access.id}/accept`
        )
        this.getListGranted()
        this.notify(
    ...
    async reinvite (emergency_access) {
      try {
        await this.$axios.$post(
          `cystack_platform/pm/emergency_access/${emergency_access.id}/reinvite`
        )
        this.getListTrusted()
        this.notify(
