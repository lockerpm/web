...
export default {
  ...
  methods: {
    ...
    getCipherImportName (cipherName, suffix = 1) {
      const newName = cipherName + (suffix === 1 ? '' : ` (${suffix}) `)
      if (this.ciphers.some(cipher => cipher.name === newName)) {
        return this.getCipherImportName(cipherName, suffix + 1)
      } else {
        return newName
      }
    },
      ...
      try {
        ...
        if (importResult.success) {
          ...
          finalImportResult.ciphers = importResult.ciphers.map(cipher => {
            return {
              ...cipher,
              name: this.getCipherImportName(cipher.name)
            }
          })
          if (this.teamId) {
