...
export class MySearchService implements SearchServiceAbstraction {
    ...
    searchSends (sends: SendView[], query: string) {
      ...
      return sends.filter(s => {
        if (s.name != null && s.name.toLowerCase().includes(query)) {
          return true
        }
        ...
        if (s.notes != null && s.notes.toLowerCase().includes(query)) {
          return true
        }
        if (s.text?.text != null && s.text.text.toLowerCase().includes(query)) {
          return true
        }
        if (s.file?.fileName != null && s.file.fileName.toLowerCase().includes(query)) {
          return true
        }
      })
    }
